...
async def extract_page_pw(page) -> list[dict]:
    ...
    return await page.evaluate(r'''(baseUrl) => {
        ...
        for (const art of articles) {
            ...
            const priceEl = art.querySelector(
                '[class*="listing__price"], [class*="price__amount"], [class*="price-value"], [data-price]'
            );
            let price = "";
            if (priceEl) {
                price = (priceEl.getAttribute("data-price") || priceEl.innerText || "").trim();
            }
            ...
            while ((node = walker.nextNode())) {
                ...
                if (txt.includes("/ month") || txt.includes("/month") || txt.includes("per month")) {
                    // walk up to find the nearest ancestor that holds the full "RM X / month" string
                    let el = node.parentElement;
                    while (el && el !== art) {
                        const full = el.innerText.replace(/\s+/g, " ").trim();
                        if (/rm\s*[\d,]+/i.test(full)) {
                            monthly = full;
                            break;
                        }
                        el = el.parentElement;
                    }
                    if (!monthly) monthly = node.textContent.trim();
                    break;
                }
            }
    ...
    async with semaphore:
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            try:
                ...
                # wait for at least one price element to be visible before extracting
                try:
                    await pg.wait_for_selector(
                        '[class*="listing__price"], [class*="price__amount"], [class*="price-value"]',
                        timeout=8_000,
                    )
                except Exception:
                    pass  # some pages may have no prices; proceed anyway
